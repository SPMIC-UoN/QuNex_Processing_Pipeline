...
qunex hcp_fmri_volume \
      --bind="${BIND_PATH},${STUDY_FOLDER}:${STUDY_FOLDER}" \
      ...
      --scheduler="SLURM,time=24:00:00,ntasks=1,cpus-per-task=1,mem-per-cpu=50000,partition=${QueueName},qos=img,gres=gpu:1,jobname=qc-fMRIVolume_${ID}"
